"""
Geocoding Module
Handles location geocoding using geopy for map visualizations.
"""
...
def geocode_location(location: str, country_hint: str = None, state_hint: str = None) -> Optional[Tuple[float, float]]:
    """
...
def geocode_dataframe(
    ...
    # Apply to dataframe
    df['lat'] = df[location_col].map(lambda x: location_coords.get(x, (None, None))[0] if x in location_coords else None)
    df['lon'] = df[location_col].map(lambda x: location_coords.get(x, (None, None))[1] if x in location_coords else None)
