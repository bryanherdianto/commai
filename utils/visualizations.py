...
Handles chart generation using Plotly for interactive visualizations.
"""
...
# Chart color scheme - modern, vibrant palette
CHART_COLORS = [
    '#6366F1',  # Indigo
    '#8B5CF6',  # Purple
    '#EC4899',  # Pink
...
# Specific colors for Sales and Profit (distinguishable)
METRIC_COLORS = {
    'Sales': '#6366F1',   # Indigo
    'Profit': '#22C55E',  # Green
}
    ...
    **kwargs
    ...
    try:
        ...
        if chart_type in ['bar', 'vertical_bar']:
            ...
            values_col = y if y else (kwargs.get('values') or df.select_dtypes(include=['number']).columns[0])
            names_col = x if x else (kwargs.get('names') or df.select_dtypes(include=['object']).columns[0])
...
def _create_map_chart(
    ...
    **kwargs
    ...
    if not is_geopy_available():
        return _create_unsupported_chart_message("Map visualization requires 'geopy' library. Install with: pip install geopy")
    
    # Auto-detect location column if not provided
    if not location_col:
        location_candidates = ['city', 'state', 'country', 'region', 'location', 'place']
        for col in df.columns:
            if any(cand in col.lower() for cand in location_candidates):
                location_col = col
                break
        if not location_col:
            location_col = df.select_dtypes(include=['object']).columns[0] if len(df.select_dtypes(include=['object']).columns) > 0 else None
    
    if not location_col or location_col not in df.columns:
        return _create_unsupported_chart_message("No location column found for map visualization.")
    ...
    if len(geo_df) == 0:
        return _create_unsupported_chart_message("Could not geocode any locations. Try using more specific location names.")
...
def detect_chart_type(question: str) -> str:
    ...
    if 'pie' in question_lower or 'distribution' in question_lower:
        ...
        return 'line'
    elif 'scatter' in question_lower or 'correlation' in question_lower or 'relationship' in question_lower:
        return 'scatter'
        ...
        return 'box'
    elif 'bar' in question_lower or 'chart' in question_lower or 'compare' in question_lower:
        return 'bar'
    else:
        return 'bar'
