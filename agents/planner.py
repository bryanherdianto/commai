...
class PlannerAgent:
    ...
    **Chart Config:**
    ...
    def create_plan(
        self, question: str, schema_info: str, conversation_context: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Creating plan for: {question}")
        ...
        try:
            ...
            logger.info("Received plan from LLM")
    ...
    def generate_sample_prompts(self, schema_info: str) -> list:
        """Generates tailored sample prompts based on the data schema."""
        prompt = f"""
        You are an expert data analyst. Based on the data schema below, generate exactly 6 highly relevant, concise, and diverse questions a user might ask to gain insights from this specific data.
        
        Guidelines for prompts:
        - Keep them short (under 10 words)
        - Use different visualization types (trend, comparison, top/bottom, correlation)
        - Reference specific column names from the schema
        - Style examples: "Sales by Category", "Top 5 Products by Profit", "Sales Trend over Time"

        Dataset Schema:
        {schema_info}

        Return ONLY the list of questions, one per line. No numbers, no intro, no bullet points.
        """
        try:
            logger.info("Generating dynamic sample prompts")
            response = self.client.models.generate_content(
                model=self.model_id,
                contents=prompt,
                config=types.GenerateContentConfig(
            ...
            prompts = [
                p.strip() for p in response.text.strip().split("\n") if p.strip()
            ]
            logger.info(f"Generated {len(prompts)} dynamic prompts")
            return prompts[:8]
        except Exception as e:
