...
Executes the plan from the Planner agent using Gemini and generates results.
The "Doing" agent that performs the actual data analysis.
"""
...
class ExecutorAgent:
    ...
    SYSTEM_PROMPT = """You are a data analysis expert using PandasAI concepts. Given a pandas DataFrame and a question,
...
"""
            ...
            if code:
                ...
                if exec_result['success']:
                    ...
                    if isinstance(data, pd.DataFrame):
                        ...
                        result['response'] = self._format_dataframe_response(df_result, plan)
                    ...
                    simple_code = self._generate_simple_code(df, plan, original_question)
                    if simple_code:
                    ...
                    if not result['success']:
                        ...
                        result['response'] = f"Error executing analysis: {exec_result['error']}"
            else:
    ...
    def _generate_code(
        ...
        prompt = f"""DataFrame columns and sample values:
{columns_str}
...
The DataFrame is already loaded as 'df'. Write ONLY executable code."""
    ...
    def _generate_simple_code(
        ...
        if 'sub-categor' in question_lower or 'subcategor' in question_lower:
            if 'unprofitable' in question_lower or 'negative' in question_lower or 'loss' in question_lower:
                return """
...
"""
        ...
        if 'year' in question_lower or '2018' in question_lower or '2019' in question_lower or '2020' in question_lower or '2021' in question_lower:
            if 'profit' in question_lower:
                return """
...
"""
        ...
        if 'trend' in question_lower or 'over time' in question_lower or 'by month' in question_lower:
...
"""
        ...
        if 'correlation' in question_lower or ('discount' in question_lower and 'profit' in question_lower):
            return """
...
"""
    ...
    def _format_numeric_response(self, value: Union[int, float], plan: Dict[str, Any]) -> str:
